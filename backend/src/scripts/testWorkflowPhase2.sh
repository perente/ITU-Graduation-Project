...
echo "===================================="
echo "StajChain Phase 2 Workflow Test"
echo "===================================="

STUDENT_EMAIL="${STUDENT_EMAIL:-studentphase2@itu.edu.tr}"
...
REJECT_INTERNSHIP_FIELD="${REJECT_INTERNSHIP_FIELD:-DONANIM}"
WORKING_DAYS_JSON="${WORKING_DAYS_JSON:-[\"MON\",\"TUE\",\"WED\",\"THU\",\"FRI\"]}"
FLEX_WEEKLY_SCHEDULE_JSON="${FLEX_WEEKLY_SCHEDULE_JSON:-[[\"TUE\",\"WED\",\"FRI\"],[\"MON\",\"TUE\",\"WED\",\"FRI\"],[\"MON\",\"TUE\",\"THU\"]]}"
DEFAULT_MIN_REQUIRED_WORKING_DAYS="20"

if [[ "$INTERNSHIP_TYPE" == "VOLUNTARY" ]]; then
  DEFAULT_MIN_REQUIRED_WORKING_DAYS="10"
fi

MIN_REQUIRED_WORKING_DAYS="${MIN_REQUIRED_WORKING_DAYS:-$DEFAULT_MIN_REQUIRED_WORKING_DAYS}"
...
END_DATE="${END_DATE:-$(calculate_end_date_for_min_workdays "$START_DATE" "$MIN_REQUIRED_WORKING_DAYS" "$WORKING_DAYS_JSON")}"
...
REJECT_END_DATE="${REJECT_END_DATE:-$(calculate_end_date_for_min_workdays "$REJECT_START_DATE" "$MIN_REQUIRED_WORKING_DAYS" "$WORKING_DAYS_JSON")}"
...
echo "Using rejectInternshipField=$REJECT_INTERNSHIP_FIELD"
echo "Using workingDays=$WORKING_DAYS_JSON"
echo "Using flexible weeklySchedule=$FLEX_WEEKLY_SCHEDULE_JSON"
...
FLEX_START_DATE="$(python3 - <<PY
from datetime import date, timedelta
end_date = date.fromisoformat("${REJECT_END_DATE}")
print((end_date + timedelta(days=1)).isoformat())
PY
)"
FLEX_END_DATE="$(python3 - <<PY
from datetime import date, timedelta
start_date = date.fromisoformat("${FLEX_START_DATE}")
print((start_date + timedelta(days=21)).isoformat())
PY
)"
FLEX_TOTAL_WORKING_DAYS="$(python3 - <<PY
import json
print(sum(len(week) for week in json.loads("""${FLEX_WEEKLY_SCHEDULE_JSON}""")))
PY
)"
echo "Resolved flexStartDate=$FLEX_START_DATE flexEndDate=$FLEX_END_DATE flexTotalWorkingDays=$FLEX_TOTAL_WORKING_DAYS"
...
echo
echo "=== Create second agreement for reject test ==="
api_call "POST" "/api/agreements" "$STUDENT_TOKEN" "{
  \"agreementId\":\"$AG_REJECT\",
  \"companyId\":\"$COMPANY_ID\",
  \"facultyId\":\"$FACULTY_ID\",
  \"startDate\":\"$REJECT_START_DATE\",
  \"endDate\":\"$REJECT_END_DATE\",
  \"internshipType\":\"$INTERNSHIP_TYPE\",
  \"internshipField\":\"$REJECT_INTERNSHIP_FIELD\",
  \"workingDays\":$WORKING_DAYS_JSON
}"
print_last_response
assert_status "201" "create reject agreement"
AG_REJECT="$(printf '%s' "$LAST_BODY" | extract_data_id)"
[[ -n "$AG_REJECT" ]] || fail "reject agreement id missing from create response"
echo "Using reject agreement id: $AG_REJECT"

echo
echo "=== Student approve second agreement ==="
api_call "POST" "/api/agreements/$AG_REJECT/approve" "$STUDENT_TOKEN"
print_last_response
assert_status "200" "student approve reject agreement"

echo
echo "=== Company reject second agreement ==="
api_call "POST" "/api/agreements/$AG_REJECT/reject" "$COMPANY_TOKEN" "{\"reason\":\"$VALID_REJECTION_REASON\"}"
print_last_response
assert_status "200" "company reject second agreement"
assert_body_contains "REJECTED" "company reject second agreement"

echo
echo "=== Student reads rejected agreement ==="
api_call "GET" "/api/agreements/$AG_REJECT" "$STUDENT_TOKEN"
print_last_response
assert_status "200" "student reads rejected agreement"
assert_body_contains "REJECTED" "student reads rejected agreement"

echo
echo "=== Student reads rejected history ==="
api_call "GET" "/api/agreements/$AG_REJECT/history" "$STUDENT_TOKEN"
print_last_response
assert_status "200" "student reads rejected history"
assert_body_contains "AgreementRejectedByCompany" "student reads rejected history"

echo
echo "=== Create flexible weekly schedule agreement ==="
api_call "POST" "/api/agreements" "$STUDENT_TOKEN" "{
  \"agreementId\":\"$AG_FLEX\",
  \"companyId\":\"$COMPANY_ID\",
  \"facultyId\":\"$FACULTY_ID\",
  \"startDate\":\"$FLEX_START_DATE\",
  \"endDate\":\"$FLEX_END_DATE\",
  \"internshipType\":\"VOLUNTARY\",
  \"internshipField\":\"BILISIM\",
  \"weeklySchedule\":$FLEX_WEEKLY_SCHEDULE_JSON
}"
print_last_response
assert_status "201" "create flexible weekly schedule agreement"
assert_body_contains "weeklySchedule" "create flexible weekly schedule agreement"
assert_body_contains "\"totalWorkingDays\":$FLEX_TOTAL_WORKING_DAYS" "create flexible weekly schedule agreement"
AG_FLEX="$(printf '%s' "$LAST_BODY" | extract_data_id)"
[[ -n "$AG_FLEX" ]] || fail "flexible weekly schedule agreement id missing from create response"
echo "Using flexible weekly schedule agreement id: $AG_FLEX"

echo
echo "=== Student approve flexible weekly schedule agreement ==="
api_call "POST" "/api/agreements/$AG_FLEX/approve" "$STUDENT_TOKEN"
print_last_response
assert_status "200" "student approve flexible weekly schedule agreement"

echo
echo "=== Company reject flexible weekly schedule agreement ==="
api_call "POST" "/api/agreements/$AG_FLEX/reject" "$COMPANY_TOKEN" "{\"reason\":\"$VALID_REJECTION_REASON\"}"
print_last_response
assert_status "200" "company reject flexible weekly schedule agreement"
assert_body_contains "REJECTED" "company reject flexible weekly schedule agreement"

echo
echo "=== DONE ==="
echo "Phase 2 workflow test completed successfully."
